...
public class RabbitMQConsumer implements Flow.Subscription{
    ...
    @RabbitListener(bindings ={
            @QueueBinding(
                    value = @Queue(value = "deportista.handles", durable = "true"),
                    exchange = @Exchange(value = "core-clubdeportivo", type = "topic"),
                    key = "clubdeportivo.deportista.#"
            ),
            @QueueBinding(
                    value = @Queue(value = "inscripcion.handles", durable = "true"),
                    exchange = @Exchange(value = "core-clubdeportivo", type = "topic"),
                    key = "clubdeportivo.inscripcion.#"
            )
    } )
    public void receievedMessageClubDeportivo(Message<String> message) {
        localReplay(message);
    }
