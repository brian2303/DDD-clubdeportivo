...
public class EventListenerSubscriber implements Flow.Subscriber<DomainEvent>{
    ...
    public void onNext(DomainEvent event) {
        ...
        applicationEventPublisher.publishEvent(event);//para un componente de la infra (handle)
        applicationEventDrive.fire(event);//para los casos de uso tipo events (Etiqueta eventListeners)
        subscription.request(1);
    }
